...
class MapLocationRepository extends Repository
{
    ...
    public function getActiveCardsForLocale(?string $locale = null): Collection
    {
        ...
        return $this->model
            ...
            ->orderBy('id')
            ->get()
            ->map(fn (MapLocation $row) => $this->rowToCardArray($row, $locale));
    }

    /**
     * @return array<string, mixed>
     */
    public function rowToCardArray(MapLocation $row, string $locale): array
    {
        ...
        if ($image !== '' && ! preg_match('#^https?://#i', $image)) {
            $image = Storage::url(ltrim(str_replace('storage/', '', $image), '/'));
        }
        if ($image === '') {
