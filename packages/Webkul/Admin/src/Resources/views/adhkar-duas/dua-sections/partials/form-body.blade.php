...
<div class="flex flex-col gap-4">
    ...
    >
        <div class="flex flex-col gap-2">
            @if ($isEdit)
                <x-admin::breadcrumbs
                    name="adhkar_duas.dua_sections.edit"
                    :entity="$section"
                />
            @else
                <x-admin::breadcrumbs name="adhkar_duas.dua_sections.create" />
            @endif
        ...
        <div class="flex flex-wrap items-center justify-end gap-x-2.5 gap-y-2">
            @if ($isEdit && bouncer()->hasPermission('adhkar_duas.dua_sections.create'))
                <a
                    href="{{ route('admin.adhkar-duas.dua-sections.create') }}"
                    class="secondary-button whitespace-nowrap"
                >
                    @lang('admin::app.settings.dua-sections.form.add-new-section')
                </a>
            @endif

            <a
                href="{{ route('admin.adhkar-duas.dua-sections.index') }}"
                class="transparent-button hover:bg-gray-200 dark:text-white dark:hover:bg-gray-800"
            >
                @lang('admin::app.settings.dua-sections.form.back')
            </a>
            ...
            >
                @lang('admin::app.settings.dua-sections.form.save')
            </button>
        </div>
    </div>
    ...
    <div class="flex gap-2.5 max-xl:flex-wrap">
        <div class="flex flex-1 flex-col gap-2 max-xl:flex-auto">
            <x-admin::accordion class="rounded-lg">
                ...
                <x-slot:content>
                    <div class="flex flex-col gap-2">
                        @include('admin::adhkar-duas.dua-sections._form-translations', [
                    ...
                    <div class="flex flex-col gap-2">
                        @if ($isEdit)
                            <div class="mb-2">
                                <p class="text-xs font-semibold uppercase text-gray-500 dark:text-gray-400">
                                    @lang('admin::app.settings.dua-sections.form.slug-readonly')
                                </p>
                                <p class="mt-1 font-mono text-sm text-gray-800 dark:text-gray-200" v-pre>
                                    {{ $section->slug }}
                                </p>
                            </div>
                        @endif
                        ...
                        <x-admin::form.control-group>
                            <x-admin::form.control-group.label class="required">
                                @lang('admin::app.settings.dua-sections.form.sort-order')
                            </x-admin::form.control-group.label>
                            <x-admin::form.control-group.control
                                type="text"
                                name="sort_order"
                                rules="required|numeric"
                                :value="old('sort_order', $isEdit ? (string) $section->sort_order : '0')"
                                :label="trans('admin::app.settings.dua-sections.form.sort-order')"
                            />
                        ...
                        <label class="flex cursor-pointer items-center gap-2 text-sm text-gray-800 dark:text-gray-200">
                            <input
                                ...
                                @if (old('status', $isEdit ? ($section->status ? '1' : '0') : '1') === '1')
                                    checked
                                @endif
                            >
                            @lang('admin::app.settings.dua-sections.form.status-active')
                        </label>
                    </div>
                </x-slot>
            </x-admin::accordion>
